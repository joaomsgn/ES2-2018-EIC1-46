...
public class ExperimentsBinaryExternalViaJAR {
  ...
  private static final int maxEvaluations = 100;
    ...
    problemList.add(new ExperimentProblem<>(new MyProblemBinaryExternalViaJAR(prob)));
		...
		for (String algor : prob.getAlgorithms()) {
			...
			case "NSGAII":
				Algorithm<List<BinarySolution>> algorithm = new NSGAIIBuilder<>(problemList.get(i).getProblem(),
						...
						new BitFlipMutation(
								1.0 / ((MyProblemBinaryExternalViaJAR) problemList.get(i).getProblem()).getNumberOfBits(0)))
